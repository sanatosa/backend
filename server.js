// server.js — Versión mejorada: mantiene la estructura original que funciona bien
// + cache de imágenes por modelo para evitar fotos faltantes ocasionales
...
app.get('/api/grupos', async (req, res) => {
  try {
    ...
    if (!fs.existsSync(xlsxPath)) {
      return res.status(500).json({ error: "No se encontró grupos.xlsx en el backend." });
    }
...
// Función para generar clave de modelo sin talla
...
async function generarExcelAsync(params, jobId) {
  try {
    ...
    const workbookGrupos = XLSX.readFile(xlsxPath);
    ...
    // --- NUEVA FUNCIONALIDAD: Cache de imágenes por modelo ---
    const imagenesCache = {};

    for (const [idx, art] of articulos_base.entries()) {
      const fila = [];
    ...
    // Inserción de imágenes CON CACHE POR MODELO
    if (!sinImagenes) {
      jobs[jobId].fase = "Insertando imágenes...";
      const limit = pLimit(5);
      await Promise.all(articulos_base.map((art, idx) => limit(async () => {
        const cod = art.codigo?.toString().trim();
        const claveModelo = claveFoto(art.descripcion);
        
        // BUSCAR EN CACHE PRIMERO
        let fotoBuffer = imagenesCache[claveModelo];
        
        // Si no está en cache, descargar
        if (!fotoBuffer) {
          fotoBuffer = await obtenerFotoArticuloAPI(cod, usuario, password, 2);
          if (fotoBuffer) {
            imagenesCache[claveModelo] = fotoBuffer; // Guardar en cache
          }
        }
        
        // Si hay imagen (de cache o descargada), insertarla
        if (fotoBuffer) {
          try {
            ...
            ws.addImage(imgId, {
              tl: { col: campos.length - 1, row: idx + 2 },
