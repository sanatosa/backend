// server.js – Versión robusta: grupos.xlsx siempre encontrado, imágenes alineadas sin desfase
...
// Endpoint: Listar grupos del archivo grupos.xlsx de forma robusta
...
// --------- Funciones auxiliares y generación de Excel ----------
...
// ---- Generación asíncrona del Excel ----
async function generarExcelAsync(params, jobId) {
  try {
    ...
    // Uso robusto de ruta absoluta para grupos.xlsx
    const xlsxPath = path.join(__dirname, 'grupos.xlsx');
    if (!fs.existsSync(xlsxPath)) {
    ...
    // (Opcional: cálculo productos promoción)
    ...
    // --- NUEVO: Mapeo explícito fila/código para insertar imágenes en la fila exacta ---
    ...
    // Mapeo entre código y número de fila (empieza en 2 tras cabecera)
    const filaPorCodigo = {};
    ...
    // Inserción de fotos SIN DESFASE gracias al mapeo fila-código
    if (!sinImagenes) {
